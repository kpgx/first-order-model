...
def print_size_of_model(_model):
    #torch.save(_model.state_dict(), "temp.p")
    torch.save(_model, "temp.p")
    print('Size (MB):', os.path.getsize("temp.p")/1e6)
    os.remove('temp.p')


def print_dtype_of_model(_model):
    print("print_dtype_of_model")
...
#    source_image = imageio.imread(opt.source_image)
    ...
    backend = "qnnpack"
    kp_detector.qconfig = torch.quantization.get_default_qconfig(backend)
    torch.backends.quantized.engine = backend
    quant_kp_detector = torch.quantization.prepare(kp_detector, inplace=False)
    quant_kp_detector = torch.quantization.convert(quant_kp_detector, inplace=False)
    #imageio.mimsave(opt.result_video, [img_as_ubyte(frame) for frame in predictions], fps=fps)
    #q_kp_detector = torch.quantization.quantize_dynamic(kp_detector, {torch.nn.Linear}, dtype=torch.qint8)
    print_size_of_model(quant_kp_detector)
    print_dtype_of_model(qunat_kp_detector)

    print_size_of_model(kp_detector)
    print_dtype_of_model(kp_detector)
    #q_kp_detector = torch.quantization.quantize_dynamic(kp_detector, dtype=torch.qint8)
