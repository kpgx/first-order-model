...
def onnx_extract_keypoints(video, kp_detector_file_name, rtime='gpu'):
    ...
    #print("**using execution provider [{}]".format(EP_list))
...
def extract_keypoints(source_image, driving_video, generator, kp_detector, relative=True, adapt_movement_scale=True, cpu=False):
	...
	# no idea what's the permute do
        source = torch.tensor(source_image[np.newaxis].astype(np.float16)).permute(0, 3, 1, 2)
        driving = torch.tensor(np.array(driving_video)[np.newaxis].astype(np.float16)).permute(0, 4, 1, 2, 3)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--checkpoint", default='int8_kpd.onnx', help="path to onnx checkpoint to restore")

#    parser.add_argument("--source_image", default='sup-mat/source.png', help="path to source image")
    parser.add_argument("--driving_video", default='sup-mat/source.png', help="path to driving video")
    parser.add_argument("--out_kp_file", default='working/keypoints', help="path to output keypoints file")
...
#    source_image = imageio.imread(opt.source_image)
    ...
    #print("ori", source_image.shape)

    #source_image = resize(source_image, (256, 256))[..., :3]
    #print("resized", source_image.shape)

    src_img_file_name = opt.out_img_file
    imageio.imwrite(src_img_file_name, source_image)
    #print("resizing video", time.time())

    #driving_video = [resize(frame, (256, 256))[..., :3] for frame in driving_video]

    #print("calling extract key points,", time.time())
